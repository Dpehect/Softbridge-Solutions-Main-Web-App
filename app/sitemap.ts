...
function getLocalizedSlug(
  ...
  if (marketRoute === "en") return originalSlug;
  return routeTranslations[originalSlug]?.[marketRoute] ?? originalSlug;
}

function getPageLocaleForMarket(defaultLocale: string): string {
  return defaultLocale.startsWith("en") ? "en" : defaultLocale;
}

function createHomepageAlternates(): Record<string, string> {
  const languages: Record<string, string> = {};
  for (const market of marketConfigs) {
    languages[market.defaultLocale] = `${siteUrl}/${market.route}`;
  }
...
function createPageAlternates(
  ...
  languages["x-default"] = `${siteUrl}/en/${originalSlug}`;
  return languages;
}

function getChangeFrequency(pageType: string): ChangeFrequency {
  if (pageType === "article" || pageType === "research") {
    return "monthly";
  }
  if (pageType === "service") return "monthly";
  return "yearly";
}
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const indexablePages = pages.filter(
    (page) =>
      page.status === "published" && page.indexable !== false,
  );
  const homepageAlternates = createHomepageAlternates();
  ...
  for (const market of marketConfigs) {
    ...
    sitemapEntries.push({
      url: `${siteUrl}/${market.route}/evidence`,
      lastModified: DEFAULT_LAST_MODIFIED,
      changeFrequency: "monthly",
      priority: 0.9,
      alternates: {
        languages: Object.fromEntries(
          marketConfigs.map((item) => [
            item.defaultLocale,
            `${siteUrl}/${item.route}/evidence`,
          ]),
        ),
      },
    });
  ...
  for (const market of marketConfigs) {
    ...
    for (const page of marketPages) {
      const isHomepage = ["", "en", "tr", "pt", "fr", "it"].includes(
        page.slug,
      );
      ...
      sitemapEntries.push({
        ...
        priority: isService
          ...
          : page.slug.includes("/")
            ...
            : 0.8,
        alternates: {
          languages: createPageAlternates(page.slug),
        },
      });
    }
  }
