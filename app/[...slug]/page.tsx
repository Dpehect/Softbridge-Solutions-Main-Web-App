...
import { pageMap, pages, primarySolutions, additionalSolutions, resourcePages, industries, siteUrl } from "../content";
...
export default async function TopicPage({params}:Props){
  ...
  const hubItems = key==="ai-solutions" ? [...primarySolutions,...additionalSolutions] : key==="resources" ? resourcePages : key==="industries" ? industries.map(x=>pageMap.get(`industries/${x.slug}`)!) : [];
  ...
  return <>
    <JsonLd page={p} crumbs={crumbs}/><SiteHeader />
    <main className="article-page">
      <header className="article-hero grid-bg">
        ...
        <p className="eyebrow">{p.eyebrow} · Lisbon, Portugal</p>
        <h1>{p.title}</h1>
